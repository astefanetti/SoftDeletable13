 ...
 * and softdeletable2 by Igor Fomin
...
class SoftDeletableBehavior extends ModelBehavior {
	...
	function beforeDelete(&$Model, $cascade = true) {
		if ($this->settings[$Model->alias]['delete'] && $Model->hasField($this->settings[$Model->alias]['field'])) {
			...
			foreach(array_merge(array_keys($data[$Model->alias]), array('field', 'field_date', 'find', 'delete', 'field_by', 'auth_session', 'user_model')) as $field) {
	...
	function purge(&$Model, $cascade = true) {
		...
		if ($Model->hasField($this->settings[$Model->alias]['field'])) {
			...
			$purged = $Model->deleteAll(array($this->settings[$Model->alias]['field'] => '0'), $cascade);
	...
	function beforeFind(&$Model, $queryData) {
		if ($this->settings[$Model->alias]['find'] && $Model->hasField($this->settings[$Model->alias]['field'])) {
			...
			$queryData['conditions'][$Model->alias . '.' . $this->settings[$Model->alias]['field'] . ' !='] = '0';
		}
